...
export function buildSessionKeyboard(sessions: string[]): InlineKeyboard {
  ...
  for (let i = 0; i < sessions.length; i++) {
    ...
    const label = sessions[i]
      .replace(/^telegram-(dm|group):/, "")
      .slice(0, 30); // Telegram button labels max ~64 bytes; keep short
...
export function parseCallbackData(data: string): CallbackAction {
  ...
  if (data.startsWith(CB_PREFIX.MODEL_SWITCH)) {
    return { type: "model_switch", model: data.slice(CB_PREFIX.MODEL_SWITCH.length) };
  }
  if (data.startsWith(CB_PREFIX.SESSION_SWITCH)) {
    return { type: "session_switch", session: data.slice(CB_PREFIX.SESSION_SWITCH.length) };
  }
